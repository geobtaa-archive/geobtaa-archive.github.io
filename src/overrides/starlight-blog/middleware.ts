...
async function getBlogSidebar(context: APIContext): Promise<StarlightRouteData['sidebar']> {
  ...
  const allEntries = await getBlogEntries(locale)
  ...
  const years = getYearBuckets(allEntries)
    ...
    sidebar.push(
      makeSidebarGroup(
        ...
        years.map(([year, count]) =>
          makeSidebarLink(
            `${year} (${count})`,
            getRelativeBlogUrl(`/year/${year}`, locale),
            isBlogYearPage(id, year),
          ),
        ),
      ),
    )
...
function getYearBuckets(entries: Awaited<ReturnType<typeof getBlogEntries>>) {
...
function isBlogYearPage(slug: string, year: number) {
  const slugLocale = getLocaleFromPath(slug)
  const yearPath = getPathWithLocale(`${config.prefix}/year/${year}`, slugLocale)
  return slug === yearPath
}
