...
var t = { async: false, baseUrl: null, breaks: false, extensions: null, gfm: true, headerIds: true, headerPrefix: "", highlight: null, hooks: null, langPrefix: "language-", mangle: true, pedantic: false, renderer: null, sanitize: false, sanitizer: null, silent: false, smartypants: false, tokenizer: null, walkTokens: null, xhtml: false };
var n = /[&<>"']/;
var s = new RegExp(n.source, "g");
var r = /[<>"']|&(?!(#\d{1,7}|#[Xx][a-fA-F0-9]{1,6}|\w+);)/;
var i = new RegExp(r.source, "g");
var l = { "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" };
var o = (e2) => l[e2];
function a(e2, t2) {
  if (t2) {
    if (n.test(e2)) return e2.replace(s, o);
  } else if (r.test(e2)) return e2.replace(i, o);
  return e2;
}
...
function f(e2, t2, n2) {
  if (e2) {
    ...
    if (0 === e3.indexOf("javascript:") || 0 === e3.indexOf("vbscript:") || 0 === e3.indexOf("data:")) return null;
  }
...
function _(e2, t2) {
  const n2 = e2.replace(/\|/g, ((e3, t3, n3) => {
    let s3 = false, r2 = t3;
    for (; --r2 >= 0 && "\\" === n3[r2]; ) s3 = !s3;
    return s3 ? "|" : " |";
  })).split(/ \|/);
  let s2 = 0;
  if (n2[0].trim() || n2.shift(), n2.length > 0 && !n2[n2.length - 1].trim() && n2.pop(), n2.length > t2) n2.splice(t2);
...
function $(e2, t2) {
  if (t2 < 1) return "";
  let n2 = "";
  for (; t2 > 1; ) 1 & t2 && (n2 += e2), t2 >>= 1, e2 += e2;
  return n2 + e2;
}
...
var S = class {
  ...
  list(e2) {
    ...
    if (t2) {
      ...
      for (l2 = 0; l2 < m2; l2++) if (this.lexer.state.top = false, k2.items[l2].tokens = this.lexer.blockTokens(k2.items[l2].text, []), !k2.loose) {
        const e3 = k2.items[l2].tokens.filter(((e4) => "space" === e4.type)), t3 = e3.length > 0 && e3.some(((e4) => /\n.*\n/.test(e4.raw)));
        k2.loose = t3;
      }
      if (k2.loose) for (l2 = 0; l2 < m2; l2++) k2.items[l2].loose = true;
      return k2;
    }
  }
  html(e2) {
  ...
  table(e2) {
    ...
    if (t2) {
      ...
      if (e3.header.length === e3.align.length) {
        ...
        for (n2 = 0; n2 < l2; n2++) /^ *-+: *$/.test(e3.align[n2]) ? e3.align[n2] = "right" : /^ *:-+: *$/.test(e3.align[n2]) ? e3.align[n2] = "center" : /^ *:-+ *$/.test(e3.align[n2]) ? e3.align[n2] = "left" : e3.align[n2] = null;
        for (l2 = e3.rows.length, n2 = 0; n2 < l2; n2++) e3.rows[n2] = _(e3.rows[n2], e3.header.length).map(((e4) => ({ text: e4 })));
        for (l2 = e3.header.length, s2 = 0; s2 < l2; s2++) e3.header[s2].tokens = this.lexer.inline(e3.header[s2].text);
        for (l2 = e3.rows.length, s2 = 0; s2 < l2; s2++) for (i2 = e3.rows[s2], r2 = 0; r2 < i2.length; r2++) i2[r2].tokens = this.lexer.inline(i2[r2].text);
        return e3;
      }
    }
  }
  ...
  escape(e2) {
    ...
    if (t2) return { type: "escape", raw: t2[0], text: a(t2[1]) };
  }
  tag(e2) {
    const t2 = this.rules.inline.tag.exec(e2);
    if (t2) return !this.lexer.state.inLink && /^<a /i.test(t2[0]) ? this.lexer.state.inLink = true : this.lexer.state.inLink && /^<\/a>/i.test(t2[0]) && (this.lexer.state.inLink = false), !this.lexer.state.inRawBlock && /^<(pre|code|kbd|script)(\s|>)/i.test(t2[0]) ? this.lexer.state.inRawBlock = true : this.lexer.state.inRawBlock && /^<\/(pre|code|kbd|script)(\s|>)/i.test(t2[0]) && (this.lexer.state.inRawBlock = false), { type: this.options.sanitize ? "text" : "html", raw: t2[0], inLink: this.lexer.state.inLink, inRawBlock: this.lexer.state.inRawBlock, text: this.options.sanitize ? this.options.sanitizer ? this.options.sanitizer(t2[0]) : a(t2[0]) : t2[0] };
  }
  link(e2) {
    ...
    if (t2) {
      ...
      if (!this.options.pedantic && /^</.test(e3)) {
        ...
        const e4 = (function(e5, t3) {
          ...
          const n3 = e5.length;
          let s3 = 0, r2 = 0;
          for (; r2 < n3; r2++) if ("\\" === e5[r2]) r2++;
          else if (e5[r2] === t3[0]) s3++;
          else if (e5[r2] === t3[1] && (s3--, s3 < 0)) return r2;
          return -1;
        })(t2[2], "()");
        if (e4 > -1) {
  ...
  emStrong(e2, t2, n2 = "") {
    ...
    if (!r2 || r2 && ("" === n2 || this.rules.inline.punctuation.exec(n2))) {
      ...
      for (a2.lastIndex = 0, t2 = t2.slice(-1 * e2.length + n3); null != (s2 = a2.exec(t2)); ) {
        if (r3 = s2[1] || s2[2] || s2[3] || s2[4] || s2[5] || s2[6], !r3) continue;
  ...
  url(e2, t2) {
    ...
    if (n2 = this.rules.inline.url.exec(e2)) {
      let e3, s2;
      if ("@" === n2[2]) e3 = a(this.options.mangle ? t2(n2[0]) : n2[0]), s2 = "mailto:" + e3;
      else {
        let t3;
        do {
...
var T = { escape: /^\\([!"#$%&'()*+,\-./:;<=>?@\[\]\\^_`{|}~])/, autolink: /^<(scheme:[^\s\x00-\x1f<>]*|email)>/, url: w, tag: "^comment|^</[a-zA-Z][\\w:-]*\\s*>|^<[a-zA-Z][\\w-]*(?:attribute)*?\\s*/?>|^<\\?[\\s\\S]*?\\?>|^<![a-zA-Z]+\\s[\\s\\S]*?>|^<!\\[CDATA\\[[\\s\\S]*?\\]\\]>", link: /^!?\[(label)\]\(\s*(href)(?:\s+(title))?\s*\)/, reflink: /^!?\[(label)\]\[(ref)\]/, nolink: /^!?\[(ref)\](?:\[\])?/, reflinkSearch: "reflink|nolink(?!\\()", emStrong: { lDelim: /^(?:\*+(?:([punct_])|[^\s*]))|^_+(?:([punct*])|([^\s_]))/, rDelimAst: /^(?:[^_*\\]|\\.)*?\_\_(?:[^_*\\]|\\.)*?\*(?:[^_*\\]|\\.)*?(?=\_\_)|(?:[^*\\]|\\.)+(?=[^*])|[punct_](\*+)(?=[\s]|$)|(?:[^punct*_\s\\]|\\.)(\*+)(?=[punct_\s]|$)|[punct_\s](\*+)(?=[^punct*_\s])|[\s](\*+)(?=[punct_])|[punct_](\*+)(?=[punct_])|(?:[^punct*_\s\\]|\\.)(\*+)(?=[^punct*_\s])/, rDelimUnd: /^(?:[^_*\\]|\\.)*?\*\*(?:[^_*\\]|\\.)*?\_(?:[^_*\\]|\\.)*?(?=\*\*)|(?:[^_\\]|\\.)+(?=[^_])|[punct*](\_+)(?=[\s]|$)|(?:[^punct*_\s\\]|\\.)(\_+)(?=[punct*\s]|$)|[punct*\s](\_+)(?=[^punct*_\s])|[\s](\_+)(?=[punct*])|[punct*](\_+)(?=[punct*])/ }, code: /^(`+)([^`]|[^`][\s\S]*?[^`])\1(?!`)/, br: /^( {2,}|\\)\n(?!\s*$)/, del: w, text: /^(`+|[^`])(?:(?= {2,}\n)|[\s\S]*?(?:(?=[\\<!\[`*_]|\b_|$)|[^ ](?= {2,}\n)))/, punctuation: /^([\spunctuation])/ };
function A(e2) {
...
function E(e2) {
  let t2, n2, s2 = "";
  const r2 = e2.length;
  for (t2 = 0; t2 < r2; t2++) n2 = e2.charCodeAt(t2), Math.random() > 0.5 && (n2 = "x" + n2.toString(16)), s2 += "&#" + n2 + ";";
  return s2;
}
...
var v = class _v {
  ...
  static lex(e2, t2) {
    return new _v(t2).lex(e2);
  }
  static lexInline(e2, t2) {
    return new _v(t2).inlineTokens(e2);
  }
  lex(e2) {
    let t2;
    for (e2 = e2.replace(/\r\n|\r/g, "\n"), this.blockTokens(e2, this.tokens); t2 = this.inlineQueue.shift(); ) this.inlineTokens(t2.src, t2.tokens);
    return this.tokens;
  }
  blockTokens(e2, t2 = []) {
    let n2, s2, r2, i2;
    for (e2 = this.options.pedantic ? e2.replace(/\t/g, "    ").replace(/^ +$/gm, "") : e2.replace(/^( *)(\t+)/gm, ((e3, t3, n3) => t3 + "    ".repeat(n3.length))); e2; ) if (!(this.options.extensions && this.options.extensions.block && this.options.extensions.block.some(((s3) => !!(n2 = s3.call({ lexer: this }, e2, t2)) && (e2 = e2.substring(n2.raw.length), t2.push(n2), true))))) if (n2 = this.tokenizer.space(e2)) e2 = e2.substring(n2.raw.length), 1 === n2.raw.length && t2.length > 0 ? t2[t2.length - 1].raw += "\n" : t2.push(n2);
    else if (n2 = this.tokenizer.code(e2)) e2 = e2.substring(n2.raw.length), s2 = t2[t2.length - 1], !s2 || "paragraph" !== s2.type && "text" !== s2.type ? t2.push(n2) : (s2.raw += "\n" + n2.raw, s2.text += "\n" + n2.text, this.inlineQueue[this.inlineQueue.length - 1].src = s2.text);
    else if (n2 = this.tokenizer.fences(e2)) e2 = e2.substring(n2.raw.length), t2.push(n2);
    else if (n2 = this.tokenizer.heading(e2)) e2 = e2.substring(n2.raw.length), t2.push(n2);
    else if (n2 = this.tokenizer.hr(e2)) e2 = e2.substring(n2.raw.length), t2.push(n2);
    else if (n2 = this.tokenizer.blockquote(e2)) e2 = e2.substring(n2.raw.length), t2.push(n2);
    else if (n2 = this.tokenizer.list(e2)) e2 = e2.substring(n2.raw.length), t2.push(n2);
    else if (n2 = this.tokenizer.html(e2)) e2 = e2.substring(n2.raw.length), t2.push(n2);
    else if (n2 = this.tokenizer.def(e2)) e2 = e2.substring(n2.raw.length), s2 = t2[t2.length - 1], !s2 || "paragraph" !== s2.type && "text" !== s2.type ? this.tokens.links[n2.tag] || (this.tokens.links[n2.tag] = { href: n2.href, title: n2.title }) : (s2.raw += "\n" + n2.raw, s2.text += "\n" + n2.raw, this.inlineQueue[this.inlineQueue.length - 1].src = s2.text);
    else if (n2 = this.tokenizer.table(e2)) e2 = e2.substring(n2.raw.length), t2.push(n2);
    else if (n2 = this.tokenizer.lheading(e2)) e2 = e2.substring(n2.raw.length), t2.push(n2);
    else {
  ...
  inlineTokens(e2, t2 = []) {
    ...
    if (this.tokens.links) {
      ...
      if (e3.length > 0) for (; null != (i2 = this.tokenizer.rules.inline.reflinkSearch.exec(a2)); ) e3.includes(i2[0].slice(i2[0].lastIndexOf("[") + 1, -1)) && (a2 = a2.slice(0, i2.index) + "[" + $("a", i2[0].length - 2) + "]" + a2.slice(this.tokenizer.rules.inline.reflinkSearch.lastIndex));
    }
    for (; null != (i2 = this.tokenizer.rules.inline.blockSkip.exec(a2)); ) a2 = a2.slice(0, i2.index) + "[" + $("a", i2[0].length - 2) + "]" + a2.slice(this.tokenizer.rules.inline.blockSkip.lastIndex);
    for (; null != (i2 = this.tokenizer.rules.inline.escapedEmSt.exec(a2)); ) a2 = a2.slice(0, i2.index + i2[0].length - 2) + "++" + a2.slice(this.tokenizer.rules.inline.escapedEmSt.lastIndex), this.tokenizer.rules.inline.escapedEmSt.lastIndex--;
    for (; e2; ) if (l2 || (o2 = ""), l2 = false, !(this.options.extensions && this.options.extensions.inline && this.options.extensions.inline.some(((s3) => !!(n2 = s3.call({ lexer: this }, e2, t2)) && (e2 = e2.substring(n2.raw.length), t2.push(n2), true))))) if (n2 = this.tokenizer.escape(e2)) e2 = e2.substring(n2.raw.length), t2.push(n2);
    ...
    else if (n2 = this.tokenizer.emStrong(e2, a2, o2)) e2 = e2.substring(n2.raw.length), t2.push(n2);
    ...
    else if (n2 = this.tokenizer.autolink(e2, E)) e2 = e2.substring(n2.raw.length), t2.push(n2);
      ...
      if (n2 = this.tokenizer.inlineText(r2, A)) e2 = e2.substring(n2.raw.length), "_" !== n2.raw.slice(-1) && (o2 = n2.raw.slice(-1)), l2 = true, s2 = t2[t2.length - 1], s2 && "text" === s2.type ? (s2.raw += n2.raw, s2.text += n2.text) : t2.push(n2);
      else if (e2) {
        const t3 = "Infinite loop on byte: " + e2.charCodeAt(0);
        if (this.options.silent) {
          console.error(t3);
...
`;
  ...
  link(e2, t2, n2) {
    if (null === (e2 = f(this.options.sanitize, this.options.baseUrl, e2))) return n2;
    let s2 = '<a href="' + e2 + '"';
...
var C = class _C {
  ...
  static parse(e2, t2) {
    return new _C(t2).parse(e2);
  }
  static parseInline(e2, t2) {
    return new _C(t2).parseInline(e2);
  }
  parse(e2, t2 = true) {
    let n2, s2, r2, i2, l2, o2, a2, c2, p2, u2, g2, d2, f2, k2, x2, m2, b2, w2, _2, y2 = "";
    const $2 = e2.length;
    for (n2 = 0; n2 < $2; n2++) if (u2 = e2[n2], this.options.extensions && this.options.extensions.renderers && this.options.extensions.renderers[u2.type] && (_2 = this.options.extensions.renderers[u2.type].call({ parser: this }, u2), false !== _2 || !["space", "hr", "heading", "code", "table", "blockquote", "list", "html", "paragraph", "text"].includes(u2.type))) y2 += _2 || "";
    else switch (u2.type) {
      case "space":
        continue;
      case "hr":
      ...
      case "code":
        ...
        continue;
      case "table":
        for (c2 = "", a2 = "", i2 = u2.header.length, s2 = 0; s2 < i2; s2++) a2 += this.renderer.tablecell(this.parseInline(u2.header[s2].tokens), { header: true, align: u2.align[s2] });
        for (c2 += this.renderer.tablerow(a2), p2 = "", i2 = u2.rows.length, s2 = 0; s2 < i2; s2++) {
          for (o2 = u2.rows[s2], a2 = "", l2 = o2.length, r2 = 0; r2 < l2; r2++) a2 += this.renderer.tablecell(this.parseInline(o2[r2].tokens), { header: false, align: u2.align[r2] });
      ...
      case "paragraph":
        ...
        continue;
      case "text":
        for (p2 = u2.tokens ? this.parseInline(u2.tokens) : u2.text; n2 + 1 < $2 && "text" === e2[n2 + 1].type; ) u2 = e2[++n2], p2 += "\n" + (u2.tokens ? this.parseInline(u2.tokens) : u2.text);
        y2 += t2 ? this.renderer.paragraph(p2) : p2;
        continue;
      default: {
        const e3 = 'Token with "' + u2.type + '" type was not found.';
        if (this.options.silent) return void console.error(e3);
        throw new Error(e3);
      }
    }
    return y2;
  ...
  parseInline(e2, t2) {
    ...
    let n2, s2, r2, i2 = "";
    const l2 = e2.length;
    for (n2 = 0; n2 < l2; n2++) if (s2 = e2[n2], this.options.extensions && this.options.extensions.renderers && this.options.extensions.renderers[s2.type] && (r2 = this.options.extensions.renderers[s2.type].call({ parser: this }, s2), false !== r2 || !["escape", "html", "link", "image", "strong", "em", "codespan", "br", "del", "text"].includes(s2.type))) i2 += r2 || "";
    else switch (s2.type) {
      case "escape":
      case "text":
        i2 += t2.text(s2.text);
  ...
  return (n2, s2, r2) => {
    ...
    const i2 = { ...s2 }, l2 = /* @__PURE__ */ (function(e3, t3, n3) {
      return (s3) => {
        ...
        if (t3) return Promise.reject(s3);
        if (!n3) throw s3;
        n3(s3);
      };
    })((s2 = { ...M.defaults, ...i2 }).silent, s2.async, r2);
    if (null == n2) return l2(new Error("marked(): input parameter is undefined or null"));
    if ("string" != typeof n2) return l2(new Error("marked(): input parameter is of type " + Object.prototype.toString.call(n2) + ", string expected"));
    if ((function(e3) {
      ...
      if (!i3 || i3.length < 3) return a2();
      if (delete s2.highlight, !o2.length) return a2();
      let c2 = 0;
    ...
    if (s2.async) return Promise.resolve(s2.hooks ? s2.hooks.preprocess(n2) : n2).then(((t3) => e2(t3, s2))).then(((e3) => s2.walkTokens ? Promise.all(M.walkTokens(e3, s2.walkTokens)).then((() => e3)) : e3)).then(((e3) => t2(e3, s2))).then(((e3) => s2.hooks ? s2.hooks.postprocess(e3) : e3)).catch(l2);
    try {
...
M.options = M.setOptions = function(e2) {
    ...
    if (e3.hooks) {
      ...
      for (const n3 in e3.hooks) {
        ...
        Z.passThroughHooks.has(n3) ? t3[n3] = (r2) => {
          if (M.defaults.async) return Promise.resolve(e3.hooks[n3].call(t3, r2)).then(((e4) => s2.call(t3, e4)));
  ...
  let n2 = [];
  for (const s2 of e2) switch (n2 = n2.concat(t2.call(M, s2)), s2.type) {
    case "table":
      for (const e3 of s2.header) n2 = n2.concat(M.walkTokens(e3.tokens, t2));
      for (const e3 of s2.rows) for (const s3 of e3) n2 = n2.concat(M.walkTokens(s3.tokens, t2));
      break;
    case "list":
...
var B = () => {
  ...
  function i2(e3, t3) {
    ...
    for (; l3 = n3.exec(e3); ) o3 = i3.test(l3[0]), !o3 && h2 && p2.push([]), a3 = e3.substring(c3, l3.index), o3 && h2 ? p2.push(a3) : p2[p2.length - 1].push(a3), c3 = n3.lastIndex, h2 = o3;
    (h2 ? p2 : p2[p2.length - 1]).push(e3.substring(c3));
    let u2 = "";
    for (let e4 = 0, n4 = p2.length; e4 < n4; e4++) p2[e4] instanceof Array ? (u2 += "<section " + t3.attributes + ">", p2[e4].forEach((function(e5) {
  ...
  function o2(e3, t3, n3) {
    ...
    if (i3 = s3.exec(o3)) {
      const n4 = i3[1];
      for (o3 = o3.substring(0, i3.index) + o3.substring(s3.lastIndex), e3.nodeValue = o3; l3 = r3.exec(n4); ) l3[2] ? t3.setAttribute(l3[1], l3[2]) : t3.setAttribute(l3[3], "");
      return true;
    }
    return false;
  }
...
reveal.js/plugin/markdown/markdown.esm.js:
   ...
   *)
*/
//# sourceMappingURL=reveal__js_plugin_markdown_markdown__esm__js.js.map
